...
import { Frame, Flex } from 'tailwind-layout-react';
...
export default function Layout({ children }: { children: React.ReactElement }) {
  ...
  const isMainNavigation = !!mainNavigation.find((nav) => nav.path === router.pathname);
  // if (router.pathname === "/") classes = "";
  if (isMainNavigation)
    classes = 'max-w-full px-4 pl-4 md:px-20 md:pl-20 xl:pl-40 py-12 mx-auto prose dark:prose-dark text-gray-500';
  else if (components[0].components.find((component) => `/${component.key}` === router.pathname))
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden lg:flex-row bg-neutral-900 dark:bg-fill">
      <Header />
      <Navigation />
      <Frame w="0.5" color="gray-900" bgColor="indigo-600" backdropBlur="2xl">
        absas
      </Frame>
      <div className="relative left-0 flex-1 p-0 overflow-auto lg:py-6 lg:pr-6 xl:overflow-hidden">
        <div
          className={cx('h-full rounded-none bg-base lg:rounded max-w-screen-2xl', isMainNavigation && 'overflow-auto')}
        >
        ...
        <div className="flex items-center justify-end flex-1">
          <Link href="/getting-started" passHref>
            <a className="relative inline-flex items-center justify-center flex-shrink-0 w-8 h-8 align-top rounded-base bg-neutral-800">
              <img className="object-cover w-full h-full rounded-base" src="/vechai.svg" alt="logo" />
            </a>
          </Link>
        </div>
      </div>
